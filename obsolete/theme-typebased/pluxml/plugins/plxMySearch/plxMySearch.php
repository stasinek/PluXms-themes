...
class plxMySearch extends plxPlugin {
	...
	<form action="<?php echo $plxMotor->urlRewrite('?'.$plxPlugin->getParam('url')) ?>" method="post" id="searchform" >
		...
		<div id="fieldset">
			<input type="text" class="search_box" name="searchfield" value="<?php echo $searchword == '' ? $plxPlugin->getParam('frmLibButton') : $searchword; ?>"  onfocus="if (this.value == '<?php echo $plxPlugin->getParam('frmLibButton') ?>') {this.value = '';}" onblur="if (this.value == '') {this.value = '<?php echo $plxPlugin->getParam('frmLibButton') ?>';}"/>
			<input type="<?php echo (is_file(str_replace('plugins/plxMySearch/plxMySearch.php',$plxMotor->aConf['racine_themes'].$plxMotor->style.'/styles/images/search.gif',__FILE__))) ? 'image" src="'. $plxMotor->urlRewrite($plxMotor->aConf['racine_themes'].$plxMotor->style).'/styles/images/search.gif' : 'submit'?>" class="submit" name="searchbutton" value="<?php echo $plxPlugin->getParam('frmLibButton') ?>"/>
		</div>
	</form>
	<?php
	}
