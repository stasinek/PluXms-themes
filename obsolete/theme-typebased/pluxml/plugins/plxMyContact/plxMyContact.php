    ...
    public function plxShowStaticListEnd() {
		...
		if($this->getParam('mnuDisplay')) {
			...
			if (in_array($plxMotor->aConf['style'],array('typebased')) || in_array(plxUtils::getValue($_COOKIE['style'], $plxMotor->style),array('typebased'))) {
				echo "<?php array_splice(\$menus, ".($this->getParam('mnuPos')-1).", 0, '<li class=\"page_item '.\$class.'\"><a href=\"'.\$this->plxMotor->urlRewrite('?contact').'\"><span>".$this->getParam('mnuName')."</span></a></li>'); ?>";
			} else {
				echo "<?php array_splice(\$menus, ".($this->getParam('mnuPos')-1).", 0, '<li class=\"page_item '.\$class.'\"><a href=\"'.\$this->plxMotor->urlRewrite('?contact').'\">".$this->getParam('mnuName')."</a></li>'); ?>";
			}
